...
public class DemoMenuTitleTest {
    ...
    void testRender(Color[] colors, String expected) {
        ...
        String dividerText = (colors.length > 1 ? "[red]" : "[green]") + "*".repeat(8);

        assertEquals(3, result.getOutput().size());
        assertEquals(dividerText, result.getOutput().get(0));
        assertEquals(expected, result.getOutput().get(1));
        assertEquals(dividerText, result.getOutput().get(2));
    }
