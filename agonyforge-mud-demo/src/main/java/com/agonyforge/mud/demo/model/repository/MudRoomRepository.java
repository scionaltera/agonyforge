...
public class MudRoomRepository extends AbstractRepository<MudRoom> {
    public static final Logger LOGGER = LoggerFactory.getLogger(MudRoomRepository.class);

    @Autowired
    public MudRoomRepository(DynamoDbClient dynamoDbClient, DynamoDbProperties.TableNames tableNames) {
        super(dynamoDbClient, tableNames, MudRoom.class);
    }

    @Override
    public MudRoom newInstance() {
        return new MudRoom();
    }

    public Optional<MudRoom> getById(Long id) {
        Map<String, Condition> filter = new HashMap<>();

        filter.put("pk", Condition.builder()
            .comparisonOperator(ComparisonOperator.EQ)
            .attributeValueList(AttributeValue.builder().s(DB_ROOM + id).build())
            .build());
        ...
        try {
            ...
            if (!response.hasItems() || response.items().size() <= 0) {
                LOGGER.warn("No rooms returned for {}", id);
                return Optional.empty();
            }

            MudRoom item = newInstance();
